...
import { useMarginTransfers } from "services/HistoryService";
...
import MarginTransfersTableRow from "./MarginTransfersTableRow";
...
const MarginTransfersTable: React.FC = () => {
	...
	const { isFetching: isTransfersLoading, data: transfers } = useMarginTransfers(loadParams);
	...
	const headerOptions: IHeader = {
		...
		columns: [
			...
			{
				name: "direction",
				label: formatMessage(historyMessages.transfers_direction),
				align: "right",
				width: "100px",
			},
		],
		...
		<div className={pageStyles.table_container}>
			...
			<Table header={headerOptions}>
				{isTransfersLoading ? (
					...
					transfers.results.map((transfer, idx) => (
						<MarginTransfersTableRow key={idx} transfer={transfer} type={activeTab} />
					))
				) : (
...
export default observer(MarginTransfersTable);
