...
import { IMarginTransferBody } from "types/exchange";
...
const TransferModal: React.FC<IProps> = ({
	...
	const {
		account: { balancesIsolated, balancesCross, balances, loadBalances },
	} = useMst();
	...
	const marginBalances = isIsolated ? balancesIsolated : balancesCross;
	const currentBalances: IBalance[] =
		formBody.from === AccountTypeEnum.SPOT
			? balances.filter((b: IBalance) => b.is_cross_margin_available)
			: marginBalances;
	...
	const pairsOptions: IOption[] = getSelectOptionsFromBalances(balancesIsolated);

	pairsOptions.forEach((o) => {
		o.label.image_png = balancesIsolated.find((b) => b.pair === o.value)?.image_png ?? "";
		o.label.image_svg = balancesIsolated.find((b) => b.pair === o.value)?.image_svg ?? "";
	});

	const balanceFrom =
		formBody.from !== AccountTypeEnum.SPOT
			? marginBalances.find(
					(b: IBalance) => b.code === (isIsolated ? formBody.currency : formBody.asset),
			  )
			: balances.find(
					(b: IBalance) => b.code === (isIsolated ? formBody.currency : formBody.asset),
			  );
	...
	useEffect(() => {
		if (formBody.from === AccountTypeEnum.SPOT) {
			...
			(!isIsolated && formBody.asset) ||
			(isIsolated && formBody.pair && formBody.currency)
		) {
			loadMarginCurrencyStatus();
		}
	}, [formBody.asset, formBody.currency, formBody.from, formBody.pair, balances]);
	...
	const loadMarginCurrencyStatus = async (): Promise<void> => {
		if ((isCross && formBody.asset) || (isIsolated && formBody.pair && formBody.currency)) {
			try {
				...
				const res: IMarginCurrencyStatus = await ExchangeService.getCurrencyStatus(params);
	...
	const handleSubmit = async () => {
		const body: IMarginTransferBody = {
		...
		if (await validate()) {
			try {
				...
				await ExchangeService.marginTransfer(body);
				loadBalances();
				loadMarginCurrencyStatus();
