...
import TxRowMobile from "./TxRowMobile";
...
const TxTable: React.FC<Props> = ({ data, loading, className, type }) => {
					...
					data.length > 0 && (
						<ExpandableList type={type}>
							{data.map((data, idx: number) => (
								<TxRowMobile key={idx} data={data} type={type} />
							))}
						</ExpandableList>
					)
				)
					...
					<Table header={headerOptions}>
						{loading ? (
							...
							data
								...
								.map((data, idx: number) => <TxRow key={idx} data={data} type={type} />)
						) : (
